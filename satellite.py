...
for i in range(length_raw):
    ...
    for key in good_satellites.keys():
        ...
        while True:
            ...
            t_s = t_v - Decimal(sqrt((s_1 - x_1)**2 + (s_2 - x_2)**2 + (s_3 - x_3)**2)) / c
            print(t_s)
            print(abs(t_v-t_s))
            #     t_s = t_s - numerator/derivative
            if abs(t_v - t_s) < Decimal(0.01)/c:

                break
              # Algorithm has a problem because once it finds the value of t_s it doesn't update s_1,s_2,s_3 to reflect that.
              #   maybe assume the opposite condition and delete those satellites who satisfy it?
        if x_1 * s_1 + x_2 * s_2 + x_3 * s_3 >= x_1 ** 2 + x_2 ** 2 + x_3 ** 2:
